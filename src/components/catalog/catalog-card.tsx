...
export function CatalogCard({ item, index }: CatalogCardProps) {
  ...
  return (
    <Link href={`/catalogo/${item.id}`} style={style} className="catalog-card group flex min-h-[27rem] flex-col overflow-hidden rounded-[1.6rem] border border-white/10 bg-slate-950/80 shadow-[0_18px_60px_rgba(0,0,0,.24)] transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-2 hover:border-blue-400/40 hover:shadow-[0_28px_80px_rgba(37,99,235,.18)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-400/30">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
          ...
          <span className="text-sm font-bold text-blue-300 opacity-0 transition-opacity group-hover:opacity-100">Ver detalhes →</span>
        </div>
      </div>
    </Link>
  );
